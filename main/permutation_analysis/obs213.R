...
#     Permutation of trait axis in the order 2, 1, 3                    #
#                                                                       #
#########################################################################
...
tr <- apply(dudi.tr$li, 2, scale)[,c(2,1,3)]
colnames(tr) <- colnames(dudi.tr$li)
######### Definition of the prior
source("main/permutation_analysis/prior_213.R")
source("lib/likelihood_exp.R")
...
save(list = c("settings", "out", "bayesianSetup", "density", "sampler", "bounds","Tmin_a_args","l_a_args","c_a_args","Tmin_b_args" ,"l_b_args"), file = paste0("results/obs_213/obs_chain", id, ".Rdata"))
