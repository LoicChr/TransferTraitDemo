...
#     Permutation of trait axis in the order 3,1,2                      #
#                                                                       #
#########################################################################
...
tr <- apply(dudi.tr$li, 2, scale)[,c(3,1,2)]
colnames(tr) <- colnames(dudi.tr$li)
######### Definition of the prior
source("main/permutation_analysis/prior_312.R")
...
save(list = c("settings", "out", "bayesianSetup", "density", "sampler", "bounds","Tmin_a_args","l_a_args","c_a_args","Tmin_b_args" ,"l_b_args"), file = paste0("results/obs_312/obs_chain", id, ".Rdata"))
