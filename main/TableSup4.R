...
TabS4 <- sapply(perms, function(perm){
  ...
  c(signif(coefs,2) , Nstep = nstep, Burn = burn, Nchains = Nchains, Gelman_msrf= signif(gelmrsf,3), logLik = signif(logLik_val,4), DIC = signif(DIC.val, 5), pseudoR2 = signif(pseudoR2, 2))
})
